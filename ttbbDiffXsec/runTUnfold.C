...
void drawRegHist(std::string outdir_, const char *data_name_, const char *h_name_,
  ...
  c->Print(Form("%s/%s_TUnfold_regular_logTaulogChi2_%s.pdf", outdir_.c_str(), data_name_, h_name_),"pdf");
  c->Clear();
  ...
  if( scanLcurve_ ){
    ...
    c->Print(Form("%s/%s_TUnfold_regular_Lcurve_%s.pdf", outdir_.c_str(), data_name_, h_name_), "pdf");
  }
  else{
    ...
    grp_BestTau->Draw("p");
    //grp_BestTau->SetEditable(false);
    c->Print(Form("%s/%s_TUnfold_regular_minRho_%s.pdf", outdir_.c_str(), data_name_, h_name_), "pdf");
  } 
}

std::vector<TH1 *> runTUnfold(std::string outdir_, const char *data_name_, TH1 *h_in_, TH2 *h_resp_,
  ...
  if( !m_sys_.empty() ){
    std::cout << "Number of sys sources: " << m_bkgs_.size() << std::endl;
  ...
  if( regMode == TUnfold::kRegModeNone ) bestTau = 0.0;
    ...
    if( !fixTau_ ){
      ...
      if( scanLcurve )
        drawRegHist(outdir_, data_name_, h_in->GetName(), spl_logTauscanX, grp_BestLcurveX, grp_BestTau, grp_Lcurve);
      else
        drawRegHist(outdir_, data_name_, h_in->GetName(), spl_logTauscanX, grp_BestLcurveX, grp_BestTau, grp_Lcurve, false, spl_Tauscan);
    }
  }
  ...
  auto h2_emat = unfold->GetEmatrixTotal(Form("%s_EmatrixTot",h_in->GetName()));
  ...
  v_output.push_back(unfold->GetEmatrixTotal(Form("%s_EmatrixTotal", h_in->GetName())));
  ...
  //TH2D *h2_emat_tau = new TH2D("","",10,0,10,10,0,10);
  //unfold->GetEmatrixSysTau(h2_emat_tau);
  //h2_emat_tau->SetName(Form("%s_EmatrixSysTau", h_in->GetName()));
  //v_output.push_back(h2_emat_tau);
  /*
  for(auto m_itr=m_sys_.begin(); m_itr != m_sys_.end(); ++m_itr){
    TH2 *h2_emat_source;
    unfold->GetEmatrixSysSource(h2_emat_source, m_itr->first);
    h2_emat_source->SetName(Form("%s_EmatrixSysSource%s", h_in->GetName(), m_itr->first));
