...
class DrawSystPlots(sls.LoadSamples):
    def __init__(self, year, lumi, root_path, hist_name, output_path):
        self.year = year
        ...
        for syst in syst_name:
            ...
            for name, file in self.samples.items():
                ...
                if i == 0:
                    ...
                    TH1Nom.Scale(xsecs[name]*self.lumi/self.genevt[name])
                    TH1Up.Scale(xsecs[name]*self.lumi/self.genevt[name])
                    TH1Down.Scale(xsecs[name]*self.lumi/self.genevt[name])
                    i += 1
                else:
                    ...
                    tmpNom.Scale(xsecs[name]*self.lumi/self.genevt[name])
                    tmpUp.Scale(xsecs[name]*self.lumi/self.genevt[name])
                    tmpDown.Scale(xsecs[name]*self.lumi/self.genevt[name])
